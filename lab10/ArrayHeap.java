...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void sink(int index) {
        ...
        while (leftIndex(index) <= size()) {
            ...
            if (i < size() && contents[i].myPriority > contents[rightIndex(index)].myPriority) {
                i++;
            }
            if (!(contents[index].myPriority > contents[i].myPriority)) {
                break;
            }
            swap(i, index);
            index = i;
